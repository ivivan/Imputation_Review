...
if __name__ == "__main__":
    ...
    ########### read all imputations for 6 models #############
    for l in Model_list:
        level_imputed_scal = f'results/{l}_level_1012_scal.npy'
        NO3_imputed_scal = f'results/{l}_NO3_1012_scal.npy'
        ...
        level_imputed_array_reshape = np.array_split(
            level_imputed_array, level_imputed_array.shape[0]//6)
    ...
    for i in range(0, X_test_left.shape[0]):
        ...
        print(X_test_left_pick[:, 1])
        print(X_test_right_pick[:, 1])
        print(y_test_pick[0])

        # dim 5 --level    dim 2---NO3
        list_before = X_test_left_pick[:, 1].tolist()
        list_middle = y_test_pick[0].tolist()
        list_after = X_test_right_pick[:, 1].tolist()
        ...
        ax.plot(ori_list, color=tableau20[6], label="Ground Truth")
