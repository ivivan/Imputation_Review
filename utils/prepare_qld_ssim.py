...
def preprocess_df(df):
    ...
    scaler_x.fit(
        df[['Conductivity', 'NO3', 'Temp']])
    df[['Conductivity', 'NO3', 'Temp']] = scaler_x.transform(df[[
            'Conductivity', 'NO3', 'Temp'
        ]])
...
def test_qld_single_station():
    ...
    x_train = x_train[:4890]
    y_train = y_train[:4890]
    x_test = x_test[:1630]
    y_test = y_test[:1630]
    x_train_len = x_train_len[:4890]
    x_train_before_len = x_train_before_len[:4890]
    x_test_len = x_test_len[:1630]
    x_test_before_len = x_test_before_len[:1630]
